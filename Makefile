...
dbshell: ## PostgreSQL shell
	docker-compose -f docker-compose.prod.yml exec postgres psql -U mutiroes_user -d mutiroes_db

# Testing
test: ## Run tests
	docker-compose -f docker-compose.prod.yml exec backend1 python manage.py test

# Scaling
scale-backend: ## Scale backend (usage: make scale-backend N=5)
	docker-compose -f docker-compose.prod.yml up -d --scale backend1=$(N) --no-recreate

scale-frontend: ## Scale frontend (usage: make scale-frontend N=3)
	docker-compose -f docker-compose.prod.yml up -d --scale frontend1=$(N) --no-recreate

scale-celery: ## Scale celery workers (usage: make scale-celery N=4)
	docker-compose -f docker-compose.prod.yml up -d --scale celery-worker1=$(N) --no-recreate
...
cleanup-tasks: ## Cleanup old Celery tasks
	docker-compose -f docker-compose.prod.yml exec backend1 python manage.py shell -c "from django_celery_beat.models import PeriodicTask; PeriodicTask.objects.filter(enabled=False).delete()"
...
backup-db: ## Backup PostgreSQL database
	docker-compose -f docker-compose.prod.yml exec -T postgres pg_dump -U mutiroes_user mutiroes_db > backup_$(shell date +%Y%m%d_%H%M%S).sql

restore-db: ## Restore database (usage: make restore-db FILE=backup.sql)
	docker-compose -f docker-compose.prod.yml exec -T postgres psql -U mutiroes_user mutiroes_db < $(FILE)
